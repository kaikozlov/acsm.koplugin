...
local stack_macro = require "openssl.include.stack"
...
_M.deep_copy_of = function(typ)
  ...
  return function(ctx)
    if stack_macro.OPENSSL_sk_deep_copy then
      return stack_macro.OPENSSL_sk_deep_copy(ctx, dup, free)
    end

    local copied = stack_macro.OPENSSL_sk_new_null()
    if copied == nil then
      return nil, "stack.deep_copy_of: OPENSSL_sk_new_null() failed"
    end

    local n = tonumber(stack_macro.OPENSSL_sk_num(ctx))
    for i = 0, n - 1 do
      local elem = stack_macro.OPENSSL_sk_value(ctx, i)
      if elem == nil then
        stack_macro.OPENSSL_sk_free(copied)
        return nil, "stack.deep_copy_of: OPENSSL_sk_value() failed"
      end

      local duped = dup(ffi_cast(typ_ptr, elem))
      if duped == nil then
        stack_macro.OPENSSL_sk_pop_free(copied, free)
        return nil, "stack.deep_copy_of: element dup() failed"
      end

      if stack_macro.OPENSSL_sk_push(copied, duped) == 0 then
        free(duped)
        stack_macro.OPENSSL_sk_pop_free(copied, free)
        return nil, "stack.deep_copy_of: OPENSSL_sk_push() failed"
      end
    end
