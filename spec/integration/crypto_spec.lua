...
describe("Crypto round-trips (real OpenSSL)", function()
    ...
    it("RSA keygen + sign", function()
        local nc = require("adobe.util.nativecrypto")
        local key = assert(nc.generate_rsa_key(1024, 65537))
...
describe("crypto.key.new()", function()
    it("returns nil + error for invalid private DER", function()
        local crypto = require("adobe.util.crypto")
        local key, err = crypto.key.new("not valid private DER")
        assert.is_nil(key)
...
describe("crypto.decodepkcs12(pk, deviceKey)", function()
    ...
    it("returns nil + error on wrong deviceKey (wrong password)", function()
        local crypto = require("adobe.util.crypto")

        local wrongKey = crypto.deviceKey.new(string.rep("\xFF", 16))
        local key, err = crypto.decodepkcs12(pkcs12_b64, wrongKey)
        assert.is_nil(key)
        assert.is_truthy(err)
        assert.is_truthy(err:find("PKCS12_parse failed", 1, true))
    end)
end)
