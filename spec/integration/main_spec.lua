...
-- Tests parseAcsmMetadata, deriveOutputPath, findUniquePath, and
-- the helper functions trimError/isActivationError.

describe("main.lua helpers", function()
    local koutil, DataStorage
    ...
    describe("parseAcsmMetadata", function()
        ...
        setup(function()
            ...
            for _, p in ipairs(PluginLoader.loaded_plugins) do
                if p.name == "acsm" then ACSM = p; break end
            end
            assert.is.truthy(ACSM, "ACSM plugin not loaded")
        end)

        it("extracts title and resource from EPUB ACSM", function()
            local fixture = "/opt/acsm.koplugin/spec/integration/fixtures/sample.acsm"
            local meta = ACSM:parseAcsmMetadata(fixture)
        ...
        it("extracts format from PDF ACSM", function()
            local fixture = "/opt/acsm.koplugin/spec/integration/fixtures/pdf_acsm/boule_de_suif.acsm"
            local meta = ACSM:parseAcsmMetadata(fixture)
            assert.is.truthy(meta)
        ...
        it("returns nil for non-existent file", function()
            local meta = ACSM:parseAcsmMetadata("/nonexistent/file.acsm")
            assert.is_nil(meta)
        end)
        ...
        it("returns nil for invalid XML", function()
            ...
            local meta = ACSM:parseAcsmMetadata(path)
            assert.is_nil(meta)
    ...
    describe("deriveOutputPath", function()
        ...
        it("uses title from ACSM metadata for EPUB", function()
            local path = ACSM:deriveOutputPath("/books/test.acsm", {
                title = "My Great Book",
        ...
        it("falls back to filename swap when no title", function()
            local path = ACSM:deriveOutputPath("/books/myfile.acsm", nil)
            assert.are.equal("/books/myfile.epub", path)
        end)

        it("falls back to filename swap with PDF format", function()
            local path = ACSM:deriveOutputPath("/books/myfile.acsm", {
                format = "application/pdf",
            })
    ...
    describe("findUniquePath", function()
        ...
        it("returns original path when file does not exist", function()
            ...
            local result = ACSM:findUniquePath(path)
